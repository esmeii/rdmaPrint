...
for benchmarkDir in $benchmarkDirs; do
  ...
  if [[ "$benchmark" == "xor" ]]; then  # XOR in parallel might caused deadlock issue
    ./$benchmark -report-all -timing -gpus=1,2,3,4 >> ../run_benchmarks.log 2>> ../run_benchmarks.err &
  else
    ./$benchmark -parallel -report-all -timing -gpus=1,2,3,4 >> ../run_benchmarks.log 2>> ../run_benchmarks.err &
  fi
  pids="$pids $!"
